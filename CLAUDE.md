...
retro-tools organizes retro game ROMs on a NextUI/MinUI handheld's SD card:
groups multi-disc games (Disc 1/2/3 ...) into their own folders and writes
the `.m3u` playlists NextUI needs to launch them and share saves across
discs. Pure Python standard library, no runtime dependencies, Python >= 3.9.
...
- **`m3u.py`** — the actual NextUI-aware planners, built on top of `discs.py`
  and `plan.py`. `system_tag()` recognizes NextUI's uppercase `(TAG)` folder
  suffix convention (e.g. `PlayStation (PS)`); `looks_like_roms_root()` tells
  a `Roms` root apart from a single system folder by whether it *contains*
  tagged folders vs. *is* one. `plan_path()` is the public entry point and
  dispatches to `plan_system_folder()` for one folder or all of them under a
  root. Three sub-planners compose the result: `_plan_group()` (loose
  multi-disc files needing a new folder + moves + playlist),
  `_plan_single_disc_folders()` (opt-in via `--single-disc-folders`, tucks
  loose single-disc bin/cue pairs into their own folder), and
  `_plan_existing_folder()` (folders the user already made by hand — writes
  only the missing `.m3u`, named after the *folder*, not the game title,
  since that's what NextUI matches on).
